			 ...
			 struct rt_sigframe_user_layout *user, int usig)
{
	...
	sigtramp = current->notifier.return_fn;
...
static void handle_signal(struct ksignal *ksig, struct pt_regs *regs)
{
	 ...
	 */
	if (!ret) {
		if (test_ti_thread_flag(task_thread_info(tsk), TIF_SINGLESTEP)) {
			struct pt_regs *regs2 = task_pt_regs(tsk);

			regs2->pstate &= ~DBG_SPSR_SS;
		}
	}
