...
#include <of/cpumask.h>
#include <of/mod_devicetable.h>
#include <of/property.h>
...
/* Debug utility */
#ifdef DEBUG
static inline void of_dump_addr(const char *s, const __be32 *addr, int na)
{
	of_dbg("%s", s);
	while (na--)
		printf(" %08x", be32_to_cpu(*(addr++)));
	printf("\n");
}
#else
static inline void of_dump_addr(const char *s, const __be32 *addr, int na) { }
