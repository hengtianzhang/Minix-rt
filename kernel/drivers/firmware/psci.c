...
static int get_set_conduit_method(struct device_node *np)
{
	...
	printf("probing for conduit method from DT.\n");
...
static void __init psci_init_migrate(void)
{
	...
	if (type == PSCI_RET_NOT_SUPPORTED) {
		printf("MIGRATE_INFO_TYPE not supported.\n");
		return;
	}

	if (type != PSCI_0_2_TOS_UP_MIGRATE &&
	    type != PSCI_0_2_TOS_UP_NO_MIGRATE) {
		printf("MIGRATE_INFO_TYPE returned unknown type (%d)\n", type);
		return;
	}
	...
	printf("Trusted OS resident on physical CPU 0x%lx\n", cpuid);
}
...
static void __init psci_init_smccc(void)
{
	 ...
	 */
	printf("SMC Calling Convention v%d.%d\n",
...
static int __init psci_0_1_init(struct device_node *np)
{
	...
	printf("Using PSCI v0.1 Function IDs from DT\n");
