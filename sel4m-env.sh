...
if [ "X$name" "==" "Xsel4m-env.sh" ]; then
    echo "Source this file (do NOT execute it!) to set the sel4m Kernel environment."
    exit
fi
...
export SEL4M_BASE=$( builtin cd "$( dirname "$dir" )" > /dev/null && pwd ${pwd_opt})
unset pwd_opt
