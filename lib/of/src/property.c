...
#include <of/of.h>
#include <of/of_device.h>
#include <of/of_graph.h>
				...
				const char *propname, int elem_size)
{
	...
	if (prop->length % elem_size != 0) {
		printf("size of %s in node %pOF is not a multiple of %d\n",
					...
					struct device_node *prev)
{
	...
	if (!prev) {
		...
		if (!port) {
			printf("graph: no port node found in %pOF\n", parent);
			return NULL;
		}
	} else {
					     ...
					     u32 port, u32 endpoint)
{
	...
	if (!endpoint_node) {
		of_dbg("no valid endpoint (%d, %d) for node %pOF\n",
			 port, endpoint, node);
		return NULL;
	}
	...
	if (!remote) {
		of_dbg("no valid remote node\n");
		return NULL;
	}

	if (!of_device_is_available(remote)) {
		of_dbg("not available for remote node\n");
		return NULL;
	}
